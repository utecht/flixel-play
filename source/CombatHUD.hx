...
class CombatHUD extends FlxTypedGroup<FlxSprite> {
	...
	public function new() {
		...
		_sprEnemy = new Enemy(_sprBack.x + 76, _sprBack.y + 16, "green");
	...
	public function initCombat(PlayerHealth:Int, E:Enemy):Void {
		...
		// setup our enemy
		_enemyMaxHealth = _enemyHealth = 4; // each enemy will have health based on their type: Type 0 = 2 health, type 1 = 4 health
		_enemyHealthBar.value = 100;	// the enemy's health bar starts at 100%
		_sprEnemy.changeEnemy(e._color);	// change our enemy's image to match their type.
